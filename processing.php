<?php
    ...
    if(isset($_POST['addFoodtoBasket'])) {
        ...
        $ordersData = mysqli_query($connectDB, "SELECT * FROM orders WHERE user_id = 1 AND orders_status IS NULL");
        $ordersItemData = mysqli_query($connectDB, "SELECT * FROM orders_item WHERE food_id = '$addFoodtoBasket' AND orders_id = (SELECT orders_id FROM orders WHERE user_id = 1 AND orders_status IS NULL)");
        if(mysqli_num_rows($ordersData) == 0) {
            mysqli_query($connectDB, "INSERT INTO orders(orders_id, user_id, orders_subtotal) VALUES ('', 1, '')");
        }
        if(mysqli_num_rows($ordersItemData) == 0 ) {
            mysqli_query($connectDB, "INSERT INTO orders_item VALUES ('$addFoodtoBasket', (SELECT orders_id FROM orders WHERE user_id = 1 AND orders_status IS NULL), 1, '')");
        } else {
            $item_quantityRow = mysqli_fetch_array(mysqli_query($connectDB, "SELECT item_quantity FROM orders_item WHERE food_id = '$addFoodtoBasket' AND orders_id = (SELECT orders_id FROM orders WHERE user_id = 1 AND orders_status IS NULL)"));
            ...
            if($item_quantityRow['item_quantity'] < $food_remainingQuantityRow['food_remainingQuantity']) {
                mysqli_query($connectDB, "UPDATE orders_item SET item_quantity = item_quantity + 1 WHERE food_id = '$addFoodtoBasket' AND orders_id = (SELECT orders_id FROM orders WHERE user_id = 1 AND orders_status IS NULL)");
            }
        }
